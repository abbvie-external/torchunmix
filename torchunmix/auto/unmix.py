from .functional import unmix_batch, calc_distance, threshold_distances, cluster_medians
...
class Unmix(object):
    ...
    def _process_batch(self, batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if not batch.shape[1] == 3:
            raise TorchUnmixException(f"Invalid channel size of {batch.shape[1]}, expected 3 (BCHW)")
